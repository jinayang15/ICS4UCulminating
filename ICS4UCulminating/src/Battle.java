...
//				System.out.println("\nREROLL MOVES\n");
	...
	public void coordinateBattle() {
		...
		if (switchPokemon) {
			...
			System.out.println("--------------------");
			System.out
					.println("THEM\t" + otherMon.getName() + " HP: " + otherMonHp + "\t Level: " + otherMon.getLevel());
			System.out.println("\n");
	...
	public void attack(Move attack, Pokemon attackMon, Pokemon defendMon) {
		...
		double stab = 1; // STAB stands for 'Same Type Attack Bonus'. If the Pokemon attacks with a move
							// that has the same type as itself, it gets this bonus
...
//				
//			}
			// These next moves are only possible by the opposing Pokemon, so there is no
			// need to check if it is from the player
					...
					if (attackMon.equals(playerMon)) {
						...
						if (otherMonSpeedCount > -6 && random == 1) {
							...
							otherMon.setDeltaSpeed(
									otherMon.getDeltaSpeed() + (int) Math.floor(otherMon.getDeltaSpeed() / 6));
						...
						if (playerMonSpeedCount > -6 && random == 1) {
							...
							playerMon.setDeltaSpeed(
									playerMon.getDeltaSpeed() + (int) Math.floor(playerMon.getDeltaSpeed() / 6));
...
//			}
				...
				if (attackMon.equals(playerMon)) {
					if (trainerMonAtkCount == 5) {
						...
						playerMon.setDeltaAttack(
								playerMon.getDeltaAttack() + (int) Math.floor(playerMon.getAttack() / 6));
					} else {
						trainerMonAtkCount += 2;
						playerMon.setDeltaAttack(
								playerMon.getDeltaAttack() + (int) (2 * Math.floor(playerMon.getAttack() / 6)));
					}
				} else {
					if (otherMonAtkCount == 5) {
						...
						otherMon.setDeltaAttack(
								otherMon.getDeltaAttack() + (int) (2 * Math.floor(otherMon.getAttack() / 6)));
					}
				}
				...
				if (hit) {
					...
					if (attackMon.equals(playerMon)) {
						if (otherMonAtkCount > -6) {
							otherMonAtkCount--;
							otherMon.setDeltaAttack(
									otherMon.getDeltaAttack() + (int) Math.floor(otherMon.getDeltaAttack() / 6));
						}
					} else {
						if (trainerMonAtkCount > -6) {
							trainerMonAtkCount--;
							playerMon.setDeltaAttack(
									playerMon.getDeltaAttack() + (int) Math.floor(playerMon.getDeltaAttack() / 6));
						}
					}
	...
	// The checkBattle method is used to see if the CURRENT battle will continue
	// (i.e. same Pokemon)
	...
	// newHp = (int)
	// Math.round((2*(playerMon.getLevel()+2)*attack.getAtkPower()*(playerMon.getAttack()/otherMon.getDef())/50+2)
	// * stab * PokeType.getTypeEffectiveness(attack.getType().getTypeNum(), type1,
	// type2))
	// For 1 defender type:
	// newHp = (int)
	// Math.round((2*(playerMon.getLevel()+2)*attack.getAtkPower()*(playerMon.getAttack()/otherMon.getDef())/50+2)
	// * stab * PokeType.getTypeEffectiveness(attack.getType().getTypeNum(),
	// type1));

	// The applyAttackChecker method is used to see which Pokemon is attacking -
	// either the player or the opponent
	// It takes in the parameters of the attacking Pokemon, the move, and the STAB
	// value.
	...
	public void applyTrainerAttack(Move attack, double stab) {
			...
			if (otherMon.getTypeList().size() == 2) {
				...
				try {
					newHp = (int) Math.round((2 * (playerMon.getLevel() + 2) * attack.getAtkPower()
							* (playerMon.getAttack() / otherMon.getDef()) / 50 + 2) * stab
		...
		// Special attacks are for special stats (special attack and special defence)
			...
			if (otherMon.getTypeList().size() == 2) {
				...
				try {
					newHp = (int) Math.round((2 * (playerMon.getLevel() + 2) * attack.getAtkPower()
							* (playerMon.getSpAtk() / otherMon.getSpDef()) / 50 + 2) * stab
							* PokeType.getTypeEffectiveness(attack.getType().getTypeNum(), type1, type2));
				} catch (IOException e) {
...
//		updateStats();
	...
	public void applyOtherAttack(Move attack, double stab) {
		...
		if (playerMon.getTypeList().size() == 2) {
			...
			try {
				newHp = (int) Math.round((2 * (otherMon.getLevel() + 2) * attack.getAtkPower()
						* (otherMon.getAttack() / playerMon.getDef()) / 50 + 2) * stab
						* PokeType.getTypeEffectiveness(attack.getType().getTypeNum(), type1, type2));
			} catch (IOException e) {
